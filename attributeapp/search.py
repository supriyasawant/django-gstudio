"""Search module with complex query parsing for Attributeapp"""
...
from attributeapp.settings import STOP_WORDS
...
def createQ(token):
    ...
    if meta == 'attribute':
        if wildcards == 'BOTH':
            return Q(attributes__title__icontains=search) | \
                    Q(attributes__slug__icontains=search)
        elif wildcards == 'START':
            return Q(attributes__title__iendswith=search) | \
                    Q(attributes__slug__iendswith=search)
        elif wildcards == 'END':
            return Q(attributes__title__istartswith=search) | \
                    Q(attributes__slug__istartswith=search)
        else:
            return Q(attributes__title__iexact=search) | \
                    Q(attributes__slug__iexact=search)
    elif meta == 'author':
        if wildcards == 'BOTH':
...
def advanced_search(pattern):
    ...
    return Attributetype.published.filter(query_parsed[0]).distinct()
