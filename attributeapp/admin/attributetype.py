"""AttributetypeAdmin for Attributeapp"""
...
from attributeapp.managers import HIDDEN
from attributeapp.managers import PUBLISHED
from attributeapp.ping import DirectoryPinger
from attributeapp.admin.forms import AttributetypeAdminForm
...
class AttributetypeAdmin(reversion.VersionAdmin):
    ...
    fieldsets = ((_('Content'), {'fields': ('title', 'content', 'parent',
                 ...
                 (_('Discussion'), {'fields': ('comment_enabled',
                                               'pingback_enabled')}),
                 (_('Publication'), {'fields': ('attributes', 'tags',
                                                'sites', 'slug')}))
    list_filter = ('parent','attributes', 'authors', 'status', 'featured',
                   'login_required', 'comment_enabled', 'pingback_enabled',
                   'creation_date', 'start_publication',
                   'end_publication', 'sites')
    list_display = ('get_title', 'get_authors', 'get_attributes',
                    'get_tags', 'get_sites',
    ...
    filter_horizontal = ('attributes', 'authors', 'related')
    ...
    def get_attributes(self, attributetype):
        """Return the attributes linked in HTML"""
        try:
            attributes = ['<a href="%s" target="blank">%s</a>' %
                          (attribute.get_absolute_url(), attribute.title)
                          for attribute in attributetype.attributes.all()]
        except NoReverseMatch:
            attributes = [attribute.title for attribute in
                          attributetype.attributes.all()]
        return ', '.join(attributes)
    get_attributes.allow_tags = True
    get_attributes.short_description = _('attribute(s)')
    ...
    def get_tags(self, attributetype):
        ...
        try:
            return ', '.join(['<a href="%s" target="blank">%s</a>' %
                              ...
                              for tag in Tag.objects.get_for_object(attributetype)])
        except NoReverseMatch:
    ...
    def save_model(self, request, attributetype, form, change):
        ...
        if attributetype.pk and not request.user.has_perm('attributeapp.can_change_author'):
    ...
    def queryset(self, request):
        ...
        if request.user.has_perm('attributeapp.can_view_all'):
            return queryset
    ...
    def formfield_for_manytomany(self, db_field, request, **kwargs):
        ...
        if db_field.name == 'authors':
            if request.user.has_perm('attributeapp.can_change_author'):
    ...
    def get_actions(self, request):
        ...
        if not request.user.has_perm('attributeapp.can_change_author') \
           or not request.user.has_perm('attributeapp.can_view_all'):
    ...
    def get_urls(self):
        ...
        urls = patterns(
            ...
            url(r'^autocomplete_tags/$', 'direct_to_template',
                {'template': 'admin/attributeapp/attributetype/autocomplete_tags.js',
                 'mimetype': 'application/javascript'},
                name='attributeapp_attributetype_autocomplete_tags'),
            url(r'^wymeditor/$', 'direct_to_template',
                {'template': 'admin/attributeapp/attributetype/wymeditor.js',
                 'mimetype': 'application/javascript'},
                name='attributeapp_attributetype_wymeditor'),
            url(r'^markitup/$', 'direct_to_template',
                {'template': 'admin/attributeapp/attributetype/markitup.js',
                 'mimetype': 'application/javascript'},
                name='attributeapp_attributetype_markitup'),)
        return urls + attributetype_admin_urls

    def _media(self):
        STATIC_URL = '%sattributeapp/' % project_settings.STATIC_URL
        media = super(AttributetypeAdmin, self).media + Media(
            ...
            js=('%sjs/jquery.js' % STATIC_URL,
                ...
                reverse('admin:attributeapp_attributetype_autocomplete_tags'),))
        ...
        if settings.WYSIWYG == 'wymeditor':
            media += Media(
                js=('%sjs/wymeditor/jquery.wymeditor.pack.js' % STATIC_URL,
                    ...
                    reverse('admin:attributeapp_attributetype_wymeditor')))
        elif settings.WYSIWYG == 'tinymce':
            from tinymce.widgets import TinyMCE
            media += TinyMCE().media + Media(
                js=(reverse('tinymce-js', args=('admin/attributeapp/attributetype',)),))
            ...
            media += Media(
                js=('%sjs/markitup/jquery.markitup.js' % STATIC_URL,
                    ...
                    reverse('admin:attributeapp_attributetype_markitup')),
                css={'all': (
