"""Test cases for Attributeapp's PingBack API"""
import cStringIO
...
from attributeapp.xmlrpc.pingback import generate_pingback_content
...
class PingBackTestCase(TestCase):
    ...
    def setUp(self):
        ...
        import attributeapp.xmlrpc.pingback
        self.original_urlopen = attributeapp.xmlrpc.pingback.urlopen
        attributeapp.xmlrpc.pingback.urlopen = self.fake_urlopen
        # Preparing site
        self.site = Site.objects.get_current()
        self.site.domain = 'localhost:8000'
        self.site.save()
        # Creating tests attributetypes
        self.author = User.objects.create_user(username='webmaster',
                                               email='webmaster@example.com')
        self.attribute = Attribute.objects.create(title='test', slug='test')
        params = {'title': 'My first attributetype',
                  'content': 'My first content',
                  'slug': 'my-first-attributetype',
                  'creation_date': datetime(2010, 1, 1),
                  'status': PUBLISHED}
        self.first_attributetype = Attributetype.objects.create(**params)
        self.first_attributetype.sites.add(self.site)
        self.first_attributetype.attributes.add(self.attribute)
        self.first_attributetype.authors.add(self.author)

        params = {'title': 'My second attributetype',
                  'content': 'My second content with link '
                  'to <a href="http://%s%s">first attributetype</a>'
                  ' and other links : %s %s.' % (
                      self.site.domain,
                      self.first_attributetype.get_absolute_url(),
        ...
        self.second_attributetype.sites.add(self.site)
        self.second_attributetype.attributes.add(self.attribute)
        self.second_attributetype.authors.add(self.author)
    ...
    def test_pingback_ping(self):
        target = 'http://%s%s' % (
            self.site.domain, self.first_attributetype.get_absolute_url())
        source = 'http://%s%s' % (
            self.site.domain, self.second_attributetype.get_absolute_url())
        ...
        self.first_attributetype.pingback_enabled = False
        self.first_attributetype.save()
        response = self.server.pingback.ping(source, target)
        self.assertEquals(response, 33)

        # Validate pingback
        self.assertEquals(self.first_attributetype.comments.count(), 0)
        self.first_attributetype.pingback_enabled = True
        self.first_attributetype.save()
        response = self.server.pingback.ping(source, target)
        self.assertEquals(
            response,
            'Pingback from %s to %s registered.' % (source, target))
        self.assertEquals(self.first_attributetype.pingbacks.count(), 1)
        self.assertTrue(self.second_attributetype.title in \
                        self.first_attributetype.pingbacks[0].user_name)
    ...
    def test_pingback_extensions_get_pingbacks(self):
        ...
        self.assertEquals(response, [
            'http://localhost:8000/2010/01/01/my-second-attributetype/'])

        comment = comments.get_model().objects.create(
            content_type=ContentType.objects.get_for_model(Attributetype),
            object_pk=self.first_attributetype.pk,
