"""Test cases for Attributeapp's url_shortener"""
...
from attributeapp import url_shortener as us_settings
from attributeapp.url_shortener.backends.default import backend as default_backend


class URLShortenerTestCase(TestCase):
    """Test cases for attributeapp.url_shortener"""
    ...
    def test_get_url_shortener(self):
        ...
        us_settings.URL_SHORTENER_BACKEND = 'attributeapp.tests.custom_url_shortener'
        try:
            with warnings.catch_warnings(record=True) as w:
                self.assertEquals(get_url_shortener(), default_backend)
                self.assertTrue(issubclass(w[-1].attribute, RuntimeWarning))
        ...
        us_settings.URL_SHORTENER_BACKEND = 'attributeapp.url_shortener'\
                                            '.backends.default'
        self.assertEquals(get_url_shortener(), default_backend)
