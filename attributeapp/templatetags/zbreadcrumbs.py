"""Breadcrumb module for Attributeapp templatetags"""
import re
...
def year_crumb(creation_date):
    ...
    return Crumb(year, reverse('attributeapp_attributetype_archive_year',
                               args=[year]))
...
def month_crumb(creation_date):
    ...
    return Crumb(month_text, reverse('attributeapp_attributetype_archive_month',
                                     args=[year, month]))
...
def day_crumb(creation_date):
    ...
    return Crumb(day, reverse('attributeapp_attributetype_archive_day',
                              args=[year, month, day]))


ATTRIBUTEAPP_ROOT_URL = lambda: reverse('attributeapp_attributetype_archive_index')

MODEL_BREADCRUMBS = {'Tag': lambda x: [Crumb(_('Tags'),
                                             reverse('attributeapp_tag_list')),
                                       Crumb(x.name)],
                     'Author': lambda x: [Crumb(_('Authors'),
                                              reverse('attributeapp_author_list')),
                                        Crumb(x.username)],
                     'Attribute': lambda x: [Crumb(
                         _('Attributes'), reverse('attributeapp_attribute_list'))] + \
                     [Crumb(anc.title, anc.get_absolute_url())
                      for anc in x.get_ancestors()] + [Crumb(x.title)],
                     'Attributetype': lambda x: [year_crumb(x.creation_date),
...
def retrieve_breadcrumbs(path, model_instance, root_name=''):
    ...
    based of the model's url handled by Attributeapp"""
    breadcrumbs = []

    if root_name:
        breadcrumbs.append(Crumb(root_name, ATTRIBUTEAPP_ROOT_URL()))
    ...
    url_components = [comp for comp in
                      path.replace(ATTRIBUTEAPP_ROOT_URL(), '').split('/') if comp]
