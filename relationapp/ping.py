"""Pings utilities for Relationapp"""
import socket
import xmlrpclib
import threading
...
class URLRessources(object):
    ...
    def __init__(self):
        ...
        self.blog_feed = '%s%s' % (self.site_url,
                                   reverse('relationapp_relationtype_latest_feed'))


class DirectoryPinger(threading.Thread):
    """Threaded Directory Pinger"""

    def __init__(self, server_name, relationtypes, timeout=10, start_now=True):
        self.results = []
    ...
    def run(self):
        """Ping relationtypes to a Directory in a Thread"""
        logger = getLogger('relationapp.ping.directory')
        socket.setdefaulttimeout(self.timeout)
        for relationtype in self.relationtypes:
            reply = self.ping_relationtype(relationtype)
            self.results.append(reply)
            logger.info('%s : %s' % (self.server_name, reply['message']))
        socket.setdefaulttimeout(None)

    def ping_relationtype(self, relationtype):
        """Ping an relationtype to a Directory"""
        relationtype_url = '%s%s' % (self.ressources.site_url,
                              relationtype.get_absolute_url())
        relations = '|'.join([c.title for c in relationtype.relations.all()])

        try:
            reply = self.server.weblogUpdates.extendedPing(
                self.ressources.current_site.name,
                self.ressources.blog_url, relationtype_url,
                self.ressources.blog_feed, relations)
        except Exception:
            try:
...
class ExternalUrlsPinger(threading.Thread):
    ...
    def __init__(self, relationtype, timeout=10, start_now=True):
        self.results = []
        self.relationtype = relationtype
        self.timeout = timeout
        self.ressources = URLRessources()
        self.relationtype_url = '%s%s' % (self.ressources.site_url,
                                   self.relationtype.get_absolute_url())
    ...
    def run(self):
        ...
        logger = getLogger('relationapp.ping.external_urls')
        socket.setdefaulttimeout(self.timeout)

        external_urls = self.find_external_urls(self.relationtype)
    ...
    def find_external_urls(self, relationtype):
        """Find external urls in an relationtype"""
        soup = BeautifulSoup(relationtype.html_content)
    ...
    def pingback_url(self, server_name, target_url):
        ...
        try:
            ...
            reply = server.pingback.ping(self.relationtype_url, target_url)
