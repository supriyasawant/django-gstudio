"""Test cases for Relationapp's url_shortener"""
...
from relationapp import url_shortener as us_settings
from relationapp.url_shortener.backends.default import backend as default_backend


class URLShortenerTestCase(TestCase):
    """Test cases for relationapp.url_shortener"""
    ...
    def test_get_url_shortener(self):
        ...
        us_settings.URL_SHORTENER_BACKEND = 'relationapp.tests.custom_url_shortener'
        try:
            with warnings.catch_warnings(record=True) as w:
                self.assertEquals(get_url_shortener(), default_backend)
                self.assertTrue(issubclass(w[-1].relation, RuntimeWarning))
        ...
        us_settings.URL_SHORTENER_BACKEND = 'relationapp.url_shortener'\
                                            '.backends.default'
        self.assertEquals(get_url_shortener(), default_backend)
