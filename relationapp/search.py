"""Search module with complex query parsing for Relationapp"""
...
from relationapp.settings import STOP_WORDS
...
def createQ(token):
    ...
    if meta == 'relation':
        if wildcards == 'BOTH':
            return Q(relations__title__icontains=search) | \
                    Q(relations__slug__icontains=search)
        elif wildcards == 'START':
            return Q(relations__title__iendswith=search) | \
                    Q(relations__slug__iendswith=search)
        elif wildcards == 'END':
            return Q(relations__title__istartswith=search) | \
                    Q(relations__slug__istartswith=search)
        else:
            return Q(relations__title__iexact=search) | \
                    Q(relations__slug__iexact=search)
    elif meta == 'author':
        if wildcards == 'BOTH':
...
def advanced_search(pattern):
    ...
    return Relationtype.published.filter(query_parsed[0]).distinct()
