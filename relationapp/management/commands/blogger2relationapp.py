"""Blogger to Relationapp command module
...
class Command(NoArgsCommand):
    ...
    into Relationapp via Google's gdata API."""
    help = 'Import a Blogger blog into Relationapp.'

    option_list = NoArgsCommand.option_list + (
        make_option('--blogger-username', dest='blogger_username', default='',
                    help='The username to login to Blogger with'),
        make_option('--relation-title', dest='relation_title', default='',
                    help='The Relationapp relation to import Blogger posts to'),
    ...
    def handle_noargs(self, **options):
        ...
        self.relation_title = options.get('relation_title')
        self.blogger_blog_id = options.get('blogger_blog_id')

        self.write_out(self.style.TITLE(
            'Starting migration from Blogger to Relationapp %s\n' % __version__))
        ...
        if default_author:
            try:
                ...
                raise CommandError(
                    'Invalid Relationapp username for default author "%s"' % \
                    default_author)
        else:
        ...
        if not self.relation_title:
            self.relation_title = raw_input(
                'Relation title for imported relationtypes: ')
            if not self.relation_title:
                raise CommandError('Invalid relation title')
    ...
    def get_relation(self):
        relation, created = Relation.objects.get_or_create(
            title=self.relation_title,
            slug=slugify(self.relation_title)[:255])
    ...
    def import_posts(self):
        ...
        for post in self.blogger_manager.get_posts(self.blogger_blog_id):
            ...
            try:
                relationtype = Relationtype.objects.get(creation_date=creation_date,
                                          slug=slug)
                output = self.style.NOTICE('> Skipped %s (already migrated)\n'
                    % relationtype)
            except Relationtype.DoesNotExist:
                relationtype = Relationtype(status=status, title=title, content=content,
                              creation_date=creation_date, slug=slug)
                if self.default_author:
                    relationtype.author = self.default_author
                relationtype.tags = ','.join([slugify(cat.term) for
                                       cat in post.relation])
                relationtype.last_update = convert_blogger_timestamp(
                    post.updated.text)
                relationtype.save()
                relationtype.sites.add(self.SITE)
                relationtype.relations.add(relation)
                relationtype.authors.add(self.default_author)
                try:
                    self.import_comments(relationtype, post)
                except gdata_service.RequestError:
                    # comments not available for this post
                    pass
                output = self.style.ITEM('> Migrated %s + %s comments\n'
                    % (relationtype.title, len(Comment.objects.for_model(relationtype))))

            self.write_out(output)

    def import_comments(self, relationtype, post):
        blog_id = self.blogger_blog_id
        post_id = get_post_id(post)
        comments = self.blogger_manager.get_comments(blog_id, post_id)
        relationtype_content_type = ContentType.objects.get_for_model(Relationtype)
        ...
        for comment in comments:
            ...
            com, created = Comment.objects.get_or_create(
                content_type=relationtype_content_type,
                object_pk=relationtype.pk,
                comment=content,
