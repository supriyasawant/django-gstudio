"""RelationtypeAdmin for Relationapp"""
...
from relationapp.managers import HIDDEN
from relationapp.managers import PUBLISHED
from relationapp.ping import DirectoryPinger
from relationapp.admin.forms import RelationtypeAdminForm
...
class RelationtypeAdmin(reversion.VersionAdmin):
    ...
    fieldsets = ((_('Content'), {'fields': ('title', 'content', 'parent',
                 ...
                 (_('Discussion'), {'fields': ('comment_enabled',
                                               'pingback_enabled')}),
                 (_('Publication'), {'fields': ('relations', 'tags',
                                                'sites', 'slug')}))
    list_filter = ('parent','relations', 'authors', 'status', 'featured',
                   'login_required', 'comment_enabled', 'pingback_enabled',
                   'creation_date', 'start_publication',
                   'end_publication', 'sites')
    list_display = ('get_title', 'get_authors', 'get_relations',
                    'get_tags', 'get_sites',
    ...
    filter_horizontal = ('relations', 'authors', 'related')
    ...
    def get_relations(self, relationtype):
        """Return the relations linked in HTML"""
        try:
            relations = ['<a href="%s" target="blank">%s</a>' %
                          (relation.get_absolute_url(), relation.title)
                          for relation in relationtype.relations.all()]
        except NoReverseMatch:
            relations = [relation.title for relation in
                          relationtype.relations.all()]
        return ', '.join(relations)
    get_relations.allow_tags = True
    get_relations.short_description = _('relation(s)')
    ...
    def get_tags(self, relationtype):
        ...
        try:
            return ', '.join(['<a href="%s" target="blank">%s</a>' %
                              ...
                              for tag in Tag.objects.get_for_object(relationtype)])
        except NoReverseMatch:
    ...
    def save_model(self, request, relationtype, form, change):
        ...
        if relationtype.pk and not request.user.has_perm('relationapp.can_change_author'):
    ...
    def queryset(self, request):
        ...
        if request.user.has_perm('relationapp.can_view_all'):
            return queryset
    ...
    def formfield_for_manytomany(self, db_field, request, **kwargs):
        ...
        if db_field.name == 'authors':
            if request.user.has_perm('relationapp.can_change_author'):
    ...
    def get_actions(self, request):
        ...
        if not request.user.has_perm('relationapp.can_change_author') \
           or not request.user.has_perm('relationapp.can_view_all'):
    ...
    def get_urls(self):
        ...
        urls = patterns(
            ...
            url(r'^autocomplete_tags/$', 'direct_to_template',
                {'template': 'admin/relationapp/relationtype/autocomplete_tags.js',
                 'mimetype': 'application/javascript'},
                name='relationapp_relationtype_autocomplete_tags'),
            url(r'^wymeditor/$', 'direct_to_template',
                {'template': 'admin/relationapp/relationtype/wymeditor.js',
                 'mimetype': 'application/javascript'},
                name='relationapp_relationtype_wymeditor'),
            url(r'^markitup/$', 'direct_to_template',
                {'template': 'admin/relationapp/relationtype/markitup.js',
                 'mimetype': 'application/javascript'},
                name='relationapp_relationtype_markitup'),)
        return urls + relationtype_admin_urls

    def _media(self):
        STATIC_URL = '%srelationapp/' % project_settings.STATIC_URL
        media = super(RelationtypeAdmin, self).media + Media(
            ...
            js=('%sjs/jquery.js' % STATIC_URL,
                ...
                reverse('admin:relationapp_relationtype_autocomplete_tags'),))
        ...
        if settings.WYSIWYG == 'wymeditor':
            media += Media(
                js=('%sjs/wymeditor/jquery.wymeditor.pack.js' % STATIC_URL,
                    ...
                    reverse('admin:relationapp_relationtype_wymeditor')))
        elif settings.WYSIWYG == 'tinymce':
            from tinymce.widgets import TinyMCE
            media += TinyMCE().media + Media(
                js=(reverse('tinymce-js', args=('admin/relationapp/relationtype',)),))
            ...
            media += Media(
                js=('%sjs/markitup/jquery.markitup.js' % STATIC_URL,
                    ...
                    reverse('admin:relationapp_relationtype_markitup')),
                css={'all': (
