"""XML-RPC methods of Relationapp Pingback"""
...
from relationapp.settings import PINGBACK_CONTENT_LENGTH
...
def pingback_ping(source, target):
    ...
    try:
        ...
        try:
            relationtype = Relationtype.published.get(
                slug=kwargs['slug'],
        ...
        except (KeyError, Relationtype.DoesNotExist):
            return TARGET_IS_NOT_PINGABLE
        ...
        comment, created = comments.get_model().objects.get_or_create(
            ...
            object_pk=relationtype.pk, user_url=source, site=site,
...
def pingback_extensions_get_pingbacks(target):
    ...
    return [pingback.user_url for pingback in relationtype.pingbacks]
